package com.techstack.dataholder.impl;
...
import com.techstack.dataholder.contract.IDataHolder;
...
public class JsonDataHolder implements IDataHolder {
	...
	private static Logger s_logger = LoggerFactory.getLogger(JsonDataHolder.class);
	...
	public JsonDataHolder(String result) {
		m_mapper = new ObjectMapper();
		try {
	...
	public JsonDataHolder(String nameSpace, String result) {
		if (nameSpace == null)
		...
		try {
			...
			String message = "Error creationg generic data Holder from json string: namespace: " + nameSpace
					+ " json string" + result;
			s_logger.error(message, e);
		} catch (IOException e) {
			String message = "Error creationg generic data Holder from json string: namespace: " + nameSpace
	...
	public JsonDataHolder(byte[] jsonInputStream) {
		m_mapper = new ObjectMapper();
		try {
	...
	public List<String> getAnObjectAsStringFromArray(String namespace, String[] breadcrumbToTheArrayElement,
			String objectKey) {
		if (namespace == null)
			namespace = m_default_Name_space;
	...
	public List<IDataHolder> getObjectsAsDataHolderFromArray(String namespace, String[] breadcrumbToTheArrayElement) {
		...
		if (arrayElement.isArray()) {
			...
			for (int i = 0; i < size; i++) {
				JsonNode jsonNode = nodeArr.get(i);
				returnList.add(new JsonDataHolder(null, jsonNode.toString()));
			}
		}
		return returnList;
	}
	...
	public boolean addObjectValue(String namespace, String[] breadcrumb, Object value) {
		...
		if (breadcrumb == null) {
			...
			lastNode = breadcrumb.length == 1 ? breadcrumb[0] : breadcrumb[breadcrumb.length - 1];
		}
			...
			if (!doMerge || lastJNode == null) {
				...
				for (Object value : values) {
					if (value instanceof Long) {
						...
						s_logger.error("Unsupported dataType namespace " + namespace + " path " + breadcrumb + " value "
								+ value);
					}
				}
				currentNode.put(lastNode, an);
